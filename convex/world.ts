...
export const gameDescriptions = query({
  ...
  handler: async (ctx, args) => {
    ...
    return { worldMap, playerDescriptions, agentDescriptions, entityDescriptions };
  },
});

/**
 * The current prose state of one entity, for the details panel.
 *
 * A separate query rather than part of `gameDescriptions` on purpose: prose is not in the world
 * document (docs/05 §8), it changes on its own cadence, and only the selected entity's is ever
 * needed. `stateVersion` in the world document is the change signal that makes this cheap.
 */
export const entityState = query({
...
/**
 * The world's common knowledge (docs/05 §5.3).
 *
 * Its own query for the same reason `entityState` is: prose is not in the world document, and
 * `commonKnowledgeVersion` there is the change signal that makes this cheap to subscribe to.
 */
export const commonKnowledge = query({
  args: { worldId: v.id('worlds') },
  handler: async (ctx, args) => {
    const state = await readEntityState(ctx.db, args.worldId, COMMON_KNOWLEDGE_ID);
    return state ?? null;
  },
});

/** The most recent exchange an entity took part in (docs/09 §6). */
export const recentInteraction = query({
  args: { worldId: v.id('worlds'), entityId: v.string() },
  handler: async (ctx, args) => {
    const turns = await ctx.db
      .query('interactionTurns')
      .withIndex('byInteraction', (q) => q.eq('worldId', args.worldId))
      .order('desc')
      .take(60);
    const mine = turns.filter((t) => t.actorId === args.entityId || t.targetId === args.entityId);
    if (mine.length === 0) {
      return [];
    }
    const latest = mine[0].interactionId;
    return mine
      .filter((t) => t.interactionId === latest)
      .sort((a, b) => a.seq - b.seq)
      .map((t) => ({ speaker: t.speaker, text: t.text, actorId: t.actorId, targetId: t.targetId }));
  },
});
