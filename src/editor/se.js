...
// s - export the sprite as a .js file
...
function tileset_px_from_index(index) {
  ...
  return [
    ret[0] * (g_ctx.tiledimx + CONFIG.tilesetpadding),
    ret[1] * (g_ctx.tiledimy + CONFIG.tilesetpadding),
  ];
}
...
function sprite_from_px(x, y) {
  ...
  let texture = new PIXI.Texture(bt, new PIXI.Rectangle(x, y, w, h));
...
class LayerContext {
  constructor(app, pane, num, mod = null) {
    ...
    this.square
      .on('pointerdown', onLevelPointerDown.bind(null, this))
...
class TilesetContext {
  constructor(app, mod = g_ctx) {
    ...
    this.square
      .on('pointerdown', onTilesetDragStart)
...
function resetPanes() {
  ...
  g_ctx.g_layers[0] = new LayerContext(
    g_ctx.g_layer_apps[0],
    document.getElementById('layer0pane'),
    0,
  );
  g_ctx.g_layer_apps[1].stage.removeChildren();
  g_ctx.g_layers[1] = new LayerContext(
    g_ctx.g_layer_apps[1],
    document.getElementById('layer1pane'),
    1,
  );
  g_ctx.g_layer_apps[2].stage.removeChildren();
  g_ctx.g_layers[2] = new LayerContext(
    g_ctx.g_layer_apps[2],
    document.getElementById('layer2pane'),
    2,
  );
  g_ctx.g_layer_apps[3].stage.removeChildren();
  g_ctx.g_layers[3] = new LayerContext(
    g_ctx.g_layer_apps[3],
    document.getElementById('layer3pane'),
    3,
  );
...
function onTilesetDragEnd(e) {
  ...
  let starttilex = Math.floor(
    (g_ctx.tileset.dragctx.startx - g_ctx.tileset.fudgex) / g_ctx.tiledimx,
  );
  let starttiley = Math.floor(
    (g_ctx.tileset.dragctx.starty - g_ctx.tileset.fudgey) / g_ctx.tiledimy,
  );
  ...
  if (g_ctx.tiledimx == 16 && g_ctx.tiledimy == 16) {
    ...
    redrawGrid();
    g_ctx.selected_tiles = [];
    g_ctx.tile_index = tileset_index_from_px(
      g_ctx.tileset.dragctx.startx - g_ctx.tileset.fudgex,
      g_ctx.tileset.dragctx.starty - g_ctx.tileset.fudgey,
    );
  }
...
function onLevelMouseover(e) {
  ...
  if (
    ...
    x > this.scrollpane.scrollLeft + CONFIG.htmlCompositePaneW
  ) {
    return;
  }
  ...
  if (this.lasttileindex != g_ctx.tile_index) {
    ...
    if (g_ctx.selected_tiles.length == 0) {
      ...
      shadowsprite = sprite_from_px(
        pxloc[0] + g_ctx.tileset.fudgex,
        pxloc[1] + g_ctx.tileset.fudgey,
      );
      shadowsprite2 = sprite_from_px(
        pxloc[0] + g_ctx.tileset.fudgex,
        pxloc[1] + g_ctx.tileset.fudgey,
      );

      shadowsprite.alpha = 0.5;
      shadowsprite2.alpha = 0.5;
      ...
      for (let i = 0; i < g_ctx.selected_tiles.length; i++) {
        ...
        const shadowsprite = sprite_from_px(
          pxloc[0] + g_ctx.tileset.fudgex,
          pxloc[1] + g_ctx.tileset.fudgey,
        );
        const shadowsprite2 = sprite_from_px(
          pxloc[0] + g_ctx.tileset.fudgex,
          pxloc[1] + g_ctx.tileset.fudgey,
        );
  ...
  if (g_ctx.dkey || g_ctx.selected_tiles.length == 0) {
    ...
    for (let index of g_ctx.selected_tiles) {
      let ti = layer.addTileLevelPx(
        xorig + index[0] * g_ctx.tiledimx,
        yorig + index[1] * g_ctx.tiledimy,
        index[2],
      );
...
function onLevelDrag(layer, e) {
  ...
  layer.dragctx.tooltip.text =
    ...
    ']\n' +
    '(' +
    Math.floor(e.global.x / g_ctx.tiledimx) +
    ',' +
    Math.floor(e.global.y / g_ctx.tiledimy) +
    ')';
        ...
        if (j === starttiley) {
          ...
          if (i === starttilex) {
            ...
            ti = layer.addTileLevelPx(
              i * g_ctx.tiledimx,
              j * g_ctx.tiledimy,
              selected_grid[column - 1][row > 0 ? 1 : 0][2],
            );
          } else {
            // middle middle
            ti = layer.addTileLevelPx(
...
function initPixiApps() {
  ...
  const level_app0 = new PIXI.Application({
    ...
    view: document.getElementById('level0'),
  });
  ...
  const level_app1 = new PIXI.Application({
    ...
    width: CONFIG.levelwidth,
    height: CONFIG.levelheight,
    view: document.getElementById('level1'),
  });
  ...
  const level_app2 = new PIXI.Application({
    ...
    width: CONFIG.levelwidth,
    height: CONFIG.levelheight,
    view: document.getElementById('level3'),
  });
  ...
  const level_app3 = new PIXI.Application({
    ...
    width: CONFIG.levelwidth,
    height: CONFIG.levelheight,
    view: document.getElementById('level4'),
  });
  ...
  g_ctx.composite_app = new PIXI.Application({
    ...
    width: CONFIG.levelwidth,
    height: CONFIG.levelheight,
    view: document.getElementById('composite'),
  });
  g_ctx.composite = new CompositeContext(g_ctx.composite_app);
  ...
  g_ctx.tileset_app = new PIXI.Application({
    width: 5632,
    height: 8672,
    view: document.getElementById('tileset'),
  });
...
const initTilesConfig = async (path = CONFIG.DEFAULTTILESETPATH) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    texture.on('loaded', function () {
      ...
      g_ctx.tilesettilew =
        numtilesandpadw +
        Math.floor((g_ctx.tilesetpxw - numtilesandpadw * tileandpad) / g_ctx.tiledimx);
      let numtilesandpadh = Math.floor(g_ctx.tilesetpxh / tileandpad);
      g_ctx.tilesettileh =
        numtilesandpadh +
        Math.floor((g_ctx.tilesetpxh - numtilesandpadh * tileandpad) / g_ctx.tiledimy);
