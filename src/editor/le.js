...
function tileset_px_from_index(index) {
  ...
  return [
    ...
    ret[1] * (g_ctx.tiledimx + CONFIG.tilesetpadding),
  ];
}
...
function sprite_from_px(x, y) {
  ...
  let texture = new PIXI.Texture(bt, new PIXI.Rectangle(x, y, g_ctx.tiledimx, g_ctx.tiledimx));
...
class LayerContext {
  ...
  addTileLevelPx(x, y, index) {
    ...
    if (g_ctx.spritesheet != null) {
      ...
      ctile.animationSpeed = 0.1;
      ctile2.animationSpeed = 0.1;
...
function loadAnimatedSpritesFromModule(mod) {
  ...
  for (let key of m.keys()) {
    ...
    PIXI.Assets.load('./' + key).then(function (sheet) {
  ...
  if (event.code == 'KeyF') {
    ...
    for (let i = 0; i < undome.length; i++) {
      ...
      if (undome[i][1] !== -1) {
        ...
        let originalTile = sprite_from_px(
          pxloc[0] + g_ctx.tileset.fudgex,
          pxloc[1] + g_ctx.tileset.fudgey,
        );
        let originalTile2 = sprite_from_px(
          pxloc[0] + g_ctx.tileset.fudgex,
          pxloc[1] + g_ctx.tileset.fudgey,
        );
...
function onLevelDrag(layer, e) {
  ...
  layer.dragctx.tooltip.text =
    ...
    ',' +
    Math.floor(e.global.y / g_ctx.tiledimx) +
    ')';
...
function initTilesSync(callme) {
  return new Promise((resolve, reject) => {
    ...
    texture.on('loaded', function () {
      ...
      g_ctx.tilesettileh =
        numtilesandpadh +
        Math.floor((g_ctx.tilesetpxh - numtilesandpadh * tileandpad) / g_ctx.tiledimx);
