...
export default function PlayerDetails({
  ...
  const agentId = playerId && [...game.world.agents.values()].find((a) => a.playerId === playerId)?.id;
  const agentDescription = agentId && game.agentDescriptions.get(agentId);
  // Prose state lives outside the world document (docs/05 §8), so it is its own subscription and
  // only for the selected player.
  const proseState = useQuery(
    api.world.entityState,
    playerId ? { worldId, entityId: playerId } : 'skip',
  );
  ...
  return (
    <>
      ...
      {!isMe && proseState && (
        <div className="my-6">
          <StateDocument state={proseState ?? undefined} />
        </div>
      )}
